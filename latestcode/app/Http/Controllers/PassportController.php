...
{
    $notification = Notification::with(['rented', 'products'])->where('user_id', $id)->where('read_at', null)->orderBy('created_at', 'DESC')->get();
...
public function all_branded()
    ...
    public function allFeatured(){
        $result = User::where('is_featured', 1)->get();
        if($result){
    ...
    public function addFeatured($id)
    {
        $result = User::find($id);
        if($result){
            $user = User::where('id', $id)->update(['is_featured' => 1]);
            $res = User::find($id);
